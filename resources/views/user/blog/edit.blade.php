...
@section('content')
    <div class="container">
        ...
        <form method="POST" action="{{ route('user.blog.update', ['blog'=>$blog->id]) }}" class="m-lg-5">
            ...
            <input type="text" name="title" id="title" class="border block p-2" value="{{$blog->title}}" required>
            <br>
            <label class="d-block" for="body">Description</label>
            <textarea id="body" name="body" cols="30" class="p-2" rows="10" required>{{$blog->body}}</textarea>
